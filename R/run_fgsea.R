...
create_ranked_list <- function(covar_filename) {
  ...
  if(type == "d"){
    fname <- file.path(output_dir, paste(fname, 'ranked_list', var, 'descending.rnk', sep = "_"))
  } else {
    fname <- file.path(output_dir, paste(fname, 'ranked_list', var, 'ascending.rnk', sep = "_"))
  }
