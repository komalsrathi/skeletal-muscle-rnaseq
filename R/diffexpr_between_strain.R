...
# Function: Differential expression analysis (between strain comparison)
# Date: 04/02/2020
...
diff.expr <- function(expr, meta, annot, type = "non_exercised", 
                      var = "strain", fc = 0, plx = 7, fname = "plot_name", 
  ...
  if(write_to_text == TRUE){
    ...
    for(i in 1:length(newList)){
      if(nrow(newList[[i]]) > 1){
        sheetname <- names(newList)[[i]]
        txt.fname <- file.path(summary.out, paste0(fname, '-',sheetname,'.txt'))
        tmp <- newList[[i]] %>%
